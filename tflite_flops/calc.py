...
def calc_flops(path):
    ...
    for i in range(graph.OperatorsLength()):
        ...
        if op_code_builtin == tflite.BuiltinOperator.CONV_2D:
            ...
            filter_shape = graph.Tensors( op.Inputs(1) ).ShapeAsNumpy()
            # flops
            flops = 2 * filter_shape[0] * filter_shape[1]
            print_flops(op_code_builtin, flops)
            ...
            out_shape = graph.Tensors( op.Outputs(0) ).ShapeAsNumpy()
            # flops
            flops = 2 * in_shape[-1] * out_shape[-2] * out_shape[-1]
            print_flops(op_code_builtin, flops)

        elif op_code_builtin == tflite.BuiltinOperator.ADD:
            out_shape = graph.Tensors( op.Outputs(0) ).ShapeAsNumpy()
            # flops
            flops = 1.0
            for s in out_shape:
        ...
        elif op_code_builtin == tflite.BuiltinOperator.SUB:
            out_shape = graph.Tensors( op.Outputs(0) ).ShapeAsNumpy()
            # flops
            flops = 1.0
            for s in out_shape:
        ...
        elif op_code_builtin == tflite.BuiltinOperator.MUL:
            out_shape = graph.Tensors( op.Outputs(0) ).ShapeAsNumpy()
            # flops
            flops = 1.0
            for s in out_shape:
        ...
        elif op_code_builtin == tflite.BuiltinOperator.DIV:
            out_shape = graph.Tensors( op.Outputs(0) ).ShapeAsNumpy()
            # flops
            flops = 1.0
            for s in out_shape:
